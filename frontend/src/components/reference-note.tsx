...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mt-3">
      <p className="text-[11px] uppercase tracking-[0.08em] text-muted-foreground/80">{title}</p>
...
export function ReferenceNote({
  ...
  const meanings: Record<string, { title: string; lines: string[] }> = {
    ...
    real: {
      ...
      lines: [t("explorer.reference.meaningRealBody", { year: reference?.base ?? "" })],
    },
    gdp: {
  ...
  const cumulativeLine =
    multiYear && (mode === "nominal" || mode === "real") && !data.split
      ? t("explorer.reference.meaningCumulative", { period: period?.label ?? "" })
      : null;

  // ————— 3. Le calcul, ses conventions, ses limites —————
  const calculation: string[] = [];
  if (mode === "index") {
    calculation.push(t("explorer.reference.trendIndexMethod", { base: trend?.base ?? "" }));
  }
  if (mode === "growth") {
    calculation.push(
  ...
  if (scale) {
    calculation.push(
      ...
      t(mode === "gdp" ? "explorer.reference.gdpMethod" : "explorer.reference.capitaMethod", {
        year: reference?.base ?? "",
  ...
  if (reference?.base != null && (mode === "real" || mode === "capita" || trend != null)) {
    calculation.push(
  ...
  if (reference?.rates_source) sources.push(reference.rates_source.toUpperCase());
