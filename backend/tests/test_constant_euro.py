"""Lot A — la source unique du facteur (constant_euro.py).
...
def test_or_eur_2025_facteur_identite(db_session):
    ...
    fs = constant_euro.factor_set(db_session, 2025)
...
def test_condition_de_validite_refuse_2026(db_session):
    ...
    seed_indices(db_session)
    assert constant_euro.factor_set(db_session, 2026) is None
...
def test_condition_de_validite_refuse_sans_taux(db_session):
    ...
    db_session.flush()
    assert constant_euro.factor_set(db_session, 2025) is None
...
def test_or_real_usd_meme_realite_economique(db_session):
    ...
    seed_indices(db_session)
    eur = constant_euro.factor_set(db_session, 2025)
    usd = constant_euro.factor_set(db_session, 2025, display_currency="USD")
...
def test_devise_affichage_sans_taux_refusee(db_session):
    ...
    seed_indices(db_session)
    assert constant_euro.factor_set(db_session, 2025, display_currency="GBP") is None
...
def test_devise_non_couverte_ignoree(db_session):
    ...
    fs = constant_euro.factor_set(db_session, 2025)
    assert fs is not None
    assert all(currency in constant_euro.COVERED for currency, _ in fs.factors)
